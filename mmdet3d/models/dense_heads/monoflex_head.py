...
class MonoFlexHead(AnchorFreeMono3DHead):
    ...
    def get_predictions(
        ...
        w = pred_reg.shape[3]
        cam2imgs = torch.stack(
            [centers2d.new_tensor(input_meta["cam2img"]) for input_meta in input_metas]
        )
    ...
    def get_targets(
        ...
        base_centers2d_list, offsets2d_list, trunc_mask_list = handle_proj_objs(
            centers2d_list, gt_bboxes_list, input_metas
        )
        ...
        batch_indices = [
            centers2d_list[0].new_full((num_ctrs[i],), i) for i in range(batch_size)
        ]
