...
class NaiveCompressor_UNet(nn.Module):
    def __init__(self, input_dim, c_compress_ratio, s_compress_ratio):
        ...
        for i in range(s_compress_ratio):
            ...
            self.downsamples.add_module(
                module_name, downsample(self.final_channels, self.final_channels)
            )

        self.upsamples = nn.Sequential()
        for i in range(s_compress_ratio):
            module_name = f"up{i}"
            self.upsamples.add_module(
                module_name, upsample(if_deconv=True, channels=self.final_channels)
            )
