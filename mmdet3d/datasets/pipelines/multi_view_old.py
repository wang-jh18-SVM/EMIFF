...
class KittiSetOrigin:
    def __init__(self, point_cloud_range):
        ...
        self.origin = (point_cloud_range[:3] + point_cloud_range[3:]) / 2.0
...
class KittiRandomFlip:
    def __call__(self, results):
        if results["flip"]:
            results["lidar2img"]["intrinsic"][0, 2] = (
            ...
            boxes_flip = np.concatenate(
                [center_flip, boxes[:, 3:6], alpha_flip[:, None]], 1
            )
...
class SunRgbdRandomFlip:
    def __call__(self, results):
        if results["flip"]:
            ...
            flip_matrix[0, 0] *= -1
            extrinsic = results["lidar2img"]["extrinsic"][0][:3, :3]
            results["lidar2img"]["extrinsic"][0][:3, :3] = (
                flip_matrix @ extrinsic @ flip_matrix.T
            )
            boxes = results["gt_bboxes_3d"].tensor.numpy()
            center = boxes[:, :3]
            alpha = boxes[:, 6]
