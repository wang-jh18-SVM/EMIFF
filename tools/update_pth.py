...
def update_backbone(model, filename, map_locaiton="cpu"):
    ...
    # embed(header='update')
    filename = (
        "/home/wangz/wangzhe21/VIMI/work_dirs/0512_VIMI/best_car_3d_0.5_epoch_10.pth"
    )
    checkpoint = torch.load(filename)
    state_dict = checkpoint["state_dict"]
    backbone_i_new = {
        k.replace("backbone_i.", ""): v
        for k, v in state_dict.items()
        if k.startswith("backbone_i.")
    }
    neck_i_new = {
        k.replace("neck_i.", ""): v
        for k, v in state_dict.items()
        if k.startswith("neck_i.")
    }
    inf_compressor_new = {
        k.replace("inf_compressor.", ""): v
        for k, v in state_dict.items()
        if k.startswith("inf_compressor.")
    }
    ms_block_inf_new = {
        k.replace("ms_block_inf.", ""): v
        for k, v in state_dict.items()
        if k.startswith("ms_block_inf.")
    }
    dcn_up_conv_i_new = {
        k.replace("dcn_up_conv_i.", ""): v
        for k, v in state_dict.items()
        if k.startswith("dcn_up_conv_i.")
    }
...
def update_backbone_vqvae_veh(model, filename, map_locaiton="cpu"):
    ...
    backbone_v_new = {
        k.replace("backbone_v.", ""): v
        for k, v in state_dict.items()
        if k.startswith("backbone_v.")
    }
    neck_v_new = {
        k.replace("neck_v.", ""): v
        for k, v in state_dict.items()
        if k.startswith("neck_v.")
    }
    dcn_up_conv_v_new = {
        k.replace("dcn_up_conv_v.", ""): v
        for k, v in state_dict.items()
        if k.startswith("dcn_up_conv_v.")
    }
