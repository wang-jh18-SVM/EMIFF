...
def gen_ImageSet_from_split_data(
    ImageSets_path, split_data_path, sensor_view="vehicle"
):
...
def gen_ImageSet_from_coop_split_data(
    ImageSets_path, split_data_path, sensor_view="vehicle"
):
    ...
    frame_pairs = load_json(
        "data/cooperative-vehicle-infrastructure/cooperative/data_info.json"
    )
    ...
    for frame_pair in frame_pairs:
        veh_frame_idx = (
            frame_pair["vehicle_image_path"].split("/")[-1].replace(".jpg", "")
        )
        if veh_frame_idx in split_data:
            inf_frame_idx = (
                frame_pair["infrastructure_image_path"]
                .split("/")[-1]
                .replace(".jpg", "")
            )
...
def seq_gen_ImageSet_from_coop_split_data(
    ImageSets_path, split_data_path, sensor_view="vehicle"
):
    split_datas = read_json(split_data_path)
    veh_test_file = ""
    ...
    frame_pairs = load_json(
        "data/DAIR_V2X_Seq/SPD/cooperative-vehicle-infrastructure/cooperative/data_info.json"
    )
