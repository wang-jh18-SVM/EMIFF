...
def label_world2vlidar(sub_root, idx):
    path_input_label_file = os.path.join(
        sub_root, "cooperative/label_world", idx + ".json"
    )
    ...
    lidar_3d_list = []
    path_novatel2world = os.path.join(
        sub_root, "vehicle-side/calib/novatel_to_world", idx + ".json"
    )
    path_lidar2novatel = os.path.join(
        sub_root, "vehicle-side/calib/lidar_to_novatel", idx + ".json"
    )
    for label_world in input_label_data:
        ...
        lidar_3d_data["3d_location"]["x"] = (
            world_8_points[0][0] + world_8_points[2][0]
        ) / 2
        lidar_3d_data["3d_location"]["y"] = (
            world_8_points[0][1] + world_8_points[2][1]
        ) / 2
        lidar_3d_data["3d_location"]["z"] = (
            world_8_points[0][2] + world_8_points[4][2]
        ) / 2
...
def gen_veh_lidar2veh_cam(source_root, target_root, label_type="lidar"):
    ...
    for c_json in tqdm(c_jsons):
        ...
        c_json["calib_v_lidar2cam_path"] = os.path.join(
            "vehicle-side/calib/lidar_to_camera", veh_idx + ".json"
        )
        c_json["calib_v_cam_intrinsic_path"] = os.path.join(
            "vehicle-side/calib/camera_intrinsic/", veh_idx + ".json"
        )
        c_json["calib_lidar_i2v_path"] = os.path.join(
...
def seq_label_coop2vlidar(sub_root, idx):
    path_input_label_file = os.path.join(
        sub_root, "cooperative/label-lidar", idx + ".json"
    )
    # path_output_label_dir = os.path.join(sub_root, 'cooperative/label/lidar')
    # if not os.path.exists(path_output_label_dir):
    #     os.makedirs(path_output_label_dir)
...
def seq_label_coop2ilidar(sub_root, veh_idx, inf_idx, system_error_offset):
    path_input_label_file = os.path.join(
        sub_root, "cooperative/label-lidar", veh_idx + ".json"
    )
    ...
    r_world2veh_lidar, t_world2veh_lidar = mul_matrix(
        veh_world2novatel_r,
        veh_world2novatel_t,
        veh_novatel2lidar_r,
        veh_novatel2lidar_t,
    )
    r_inf_lidar2veh_lidar, t_inf_lidar2veh_lidar = mul_matrix(
        r_inf_lidar2world, t_inf_lidar2world, r_world2veh_lidar, t_world2veh_lidar
    )
    t_inf_lidar2veh_lidar = t_inf_lidar2veh_lidar + np.array(
        [system_error_offset["delta_x"], system_error_offset["delta_y"], 0]
    ).reshape(3, 1)

    r_veh_lidar2inf_lidar, t_veh_lidar2inf_lidar = rev_matrix(
        r_inf_lidar2veh_lidar, t_inf_lidar2veh_lidar
    )
...
def seq_gen_veh_lidar2inf_cam(source_root, target_root, label_type="lidar"):
    ...
    for c_json in tqdm(c_jsons):
        ...
        c_json["calib_i_lidar2cam_path"] = os.path.join(
            "infrastructure-side/calib/virtuallidar_to_camera", inf_idx + ".json"
        )
        ...
        label_veh_lidar_list = seq_label_coop2ilidar(
            dair_v2x_c_root, veh_idx, inf_idx, system_error_offset
        )
...
def seq_gen_calib2kitti_coop(source_root, target_root, label_type="lidar"):
    ...
    for c_json in tqdm(c_jsons):
        ...
        r_inf_lidar2world, t_inf_lidar2world = get_calibs(inf_lidar2world_path)
        r_world2inf_lidar, t_world2inf_lidar = rev_matrix(
            r_inf_lidar2world, t_inf_lidar2world
        )
        ...
        c_json["calib_i_cam_intrinsic_path"] = os.path.join(
            "infrastructure-side/calib/camera_intrinsic/", inf_idx + ".json"
        )

        # fetch transformation matrixs
        # intrinstic 3*3
        veh_cam_D, veh_cam_K = get_cam_D_and_cam_K(
            os.path.join(dair_v2x_c_root, c_json["calib_v_cam_intrinsic_path"])
        )
        inf_cam_D, inf_cam_K = get_cam_D_and_cam_K(
            os.path.join(dair_v2x_c_root, c_json["calib_i_cam_intrinsic_path"])
        )

        calib_veh_lidar2cam = read_json(
            os.path.join(dair_v2x_c_root, c_json["calib_v_lidar2cam_path"])
        )
        r_veh_lidar2cam, t_veh_lidar2cam = get_lidar2cam(calib_veh_lidar2cam)

        calib_inf_lidar2cam = read_json(
            os.path.join(dair_v2x_c_root, c_json["calib_i_lidar2cam_path"])
        )
        ...
        P2, veh_Tr_velo_to_cam = convert_calib_v2x_to_kitti(
            veh_cam_D, veh_cam_K, t_veh_lidar2cam, r_veh_lidar2cam
        )
        P3, inf_Tr_velo_to_cam = convert_calib_v2x_to_kitti(
            inf_cam_D, inf_cam_K, t_inf_lidar2cam, r_inf_lidar2cam
        )
        _, inf_lidar2veh_lidar = convert_calib_v2x_to_kitti(
            veh_cam_D, veh_cam_K, t_inf_lidar2veh_lidar, r_inf_lidar2veh_lidar
        )
        _, world2veh_lidar = convert_calib_v2x_to_kitti(
            veh_cam_D, veh_cam_K, t_world2veh_lidar, r_world2veh_lidar
        )
        _, world2inf_lidar = convert_calib_v2x_to_kitti(
            veh_cam_D, veh_cam_K, t_world2inf_lidar, r_world2inf_lidar
        )
        ...
        for ii in range(11):
            ...
            str_veh_Tr_velo_to_cam = (
                str_veh_Tr_velo_to_cam + str(veh_Tr_velo_to_cam[ii]) + " "
            )
            str_inf_Tr_velo_to_cam = (
                str_inf_Tr_velo_to_cam + str(inf_Tr_velo_to_cam[ii]) + " "
            )
            str_inf_lidar2veh_lidar = (
                str_inf_lidar2veh_lidar + str(inf_lidar2veh_lidar[ii]) + " "
            )
