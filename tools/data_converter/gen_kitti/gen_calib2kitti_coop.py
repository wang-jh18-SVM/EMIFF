...
def gen_lidar2cam_i(source_root, target_root, label_type="lidar"):
    ...
    for elem in tqdm(frame_pairs):
        ...
        label_no_filt = Label_kitti(
            osp.join(path, elem["cooperative_label_path"]), None
        )
        ...
        r_velo2cam, t_velo2cam = get_lidar2cam_2(
            osp.join(
                path,
                "infrastructure-side",
                inf_frame["calib_virtuallidar_to_camera_path"],
            )
        )
...
def gen_calib2kitti_coop(source_root, target_root, label_type="lidar"):
    ...
    for elem in tqdm(frame_pairs):
        ...
        veh_cam_D, veh_cam_K = get_cam_D_and_cam_K(
            osp.join(path, "vehicle-side", veh_frame["calib_camera_intrinsic_path"])
        )
        inf_cam_D, inf_cam_K = get_cam_D_and_cam_K(
            osp.join(
                path, "infrastructure-side", inf_frame["calib_camera_intrinsic_path"]
            )
        )
        ...
        _, world2inf_lidar = convert_calib_v2x_to_kitti(
            inf_cam_D, inf_cam_K, t_world2inf_lidar, r_world2inf_lidar
        )
